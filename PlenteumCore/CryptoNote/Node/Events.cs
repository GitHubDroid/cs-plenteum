            ...
            try
            {
                ...
                switch (Packet.Type)
                {
                    ...
                    case PacketType.NOTIFY_CHAIN_RESPONSE:
                        HandleNotifyChain(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_TX_POOL_REQUEST:
                        HandleNotifyTxPool(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_GET_OBJECTS_REQUEST:
                        HandleNotifyObjects(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_GET_OBJECTS_RESPONSE:
                        HandleNotifyObjects(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_MISSING_TXS:
                        HandleNotifyObjects(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_NEW_TRANSACTIONS:
                        HandleNotifyNewTx(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_NEW_BLOCK:
                        HandleNotifyNewBlock(Peer, Packet);
                        break;
                    case PacketType.NOTIFY_NEW_LITE_BLOCK:
                        HandleNotifyNewBlock(Peer, Packet);
                        break;
