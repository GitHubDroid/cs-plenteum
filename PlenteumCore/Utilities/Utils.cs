...
namespace Plenteum
{
    ...
    public static class Utils
    {
        ...
        public static IEnumerable<string> SplitToHashes(this string str)
        {
            var hashes = new List<string>();
            try
            {
                for (int i = 0; i < str.Length; i += 64)
                    if (str.Length - i >= 64) hashes.Add(str.Substring(i, 64));
                    else hashes.Add(str[i..]);
            }
            catch { } //ignore errors here and handle empty array at usage
            return hashes;
        }
